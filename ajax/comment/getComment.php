...
if (isset($_POST['parent_id'], $_POST['product_id'])) {
    $id = $_POST['product_id'];
    $id_comment = $_POST['parent_id'];
    // truy vấn
    $queryGetComment = "SELECT * FROM comment WHERE id = $id_comment";
    $resultGetComment = $conn->query($queryGetComment);
    $GetComment = $resultGetComment->fetch_assoc();
}
?>
<!-- respone -->
    ...
    ?>
        <li>
            <div class="d-flex align-items-center form-main-comment">
                ...
                <form action="javascript:void(0)" method="POST">
                    <input type="text" class="comment-input comment-main" name="content-main" placeholder="Viết bình luận..." value="<?php echo $GetComment['content']?>">
                    <input type="submit" name="<?php echo $id; ?>,<?php echo $id_comment; ?>,<?php echo $_SESSION['arUser']['id']; ?>" class="d-none comment-update">
                </form>
            </div>
        </li>
    <?php
